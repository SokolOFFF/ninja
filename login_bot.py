...
def handle_track(message):
    if message.chat.id in database.get_users():
        if not(message.chat.id in database.get_subscribers()):
...
def handle_settings(message):
    if message.chat.id in database.get_users():
        bot.send_message(message.chat.id, text="alright, write number and threshold in percentages (with space between)")
            ...
            if message.chat.id in database.isSettingAmount:
                try:
                    if database.isSettingAmount[message.chat.id] == True:
                        ...
                        bot.send_message(message.chat.id, text=f"you set {database.get_moneyamount(message.chat.id)} of RUB(s) to check with threshold {database.get_threshold(message.chat.id)}%")
                        database.isSettingAmount[message.chat.id] = False
                    else:
                        error(message)
